...
async def contact_received(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, "Будь ласка, введіть ПІБ дитини:", reply_markup=back_markup)
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, "Будь ласка, введіть дату народження дитини у форматі ДД.ММ.РРРР:")
...
async def process_dob(message: types.Message, state: FSMContext):
    ...
    confirmation_message = (
        ...
        f"<b>ПІБ дитини:</b> {data['real_name']}\n"
...
async def process_patient_name(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id, "Будь ласка, введіть ПІБ вашого лікаря:")
    await MyAnotherState.waiting_for_doctor_name.set()

@dp.message_handler(state=MyAnotherState.waiting_for_doctor_name)
async def process_doctor_name(message: types.Message, state: FSMContext):
    ...
    doctor_real_name = message.text
    await state.update_data(doctor_real_name=doctor_real_name)

    await bot.send_message(message.chat.id, "Будь ласка, введіть бажаний час та дату прийому:")
...
async def ask_question(message: types.Message):
    ...
    if not await state.get_state():
        ...
        await message.answer("Тут ви можете залишити питання, яке вас цікавить, і наші лікарі миттєво дадуть відповідь на нього.", reply_markup=keyboard)
...
async def questions_handler(message: types.Message):
    if message.from_user.id in admins:
        ...
        if not unanswered_questions:
            await message.answer("Немає невідповіданих питань.")
        else:
